...
@target MZ
@author ポテトドラゴン
@url https://raw.githubusercontent.com/potagon/RPGMakerMZ/master/plugins/Potagon_CreateShop.js
...
(() => {
    ...
    const BuyOnly    = Potagon.convertBool(params.BuyOnly);
    ...
    const MiniWindow = Potagon.convertBool(params.MiniWindow);
    const SubCommand = Potagon.convertBool(params.SubCommand);
    ...
    function create_shop(good_lists, buy_only) {
        ...
        for (let i = 0; i < good_lists.length; i++) {
            ...
            for (let j = 0; j < material_lists.length; j++) {
                ...
                let item  = Potagon.itemSearch(material.name);
