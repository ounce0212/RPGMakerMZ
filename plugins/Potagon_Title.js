...
@target MZ
@author ポテトドラゴン
@url https://raw.githubusercontent.com/potagon/RPGMakerMZ/master/plugins/Potagon_Title.js
...
(() => {
    ...
    // 各パラメータ用変数
    const SelectOnlyNewGame = Potagon.convertBool(params.SelectOnlyNewGame);
    const FixedTitle        = Potagon.convertBool(params.FixedTitle);
    const FixedTitleName    = String(params.FixedTitleName) || '';
    const SubTitle          = Potagon.convertBool(params.SubTitle);
    const SubTitleName      = String(params.SubTitleName) || '';
    const Version           = Potagon.convertBool(params.Version);
    const VersionName       = String(params.VersionName) || 'ver';
    const VersionPos        = Number(params.VersionPos || 1);
    const VersionId         = Potagon.convertBool(params.VersionId);
