...
@target MZ
@author ポテトドラゴン
@url https://raw.githubusercontent.com/potagon/RPGMakerMZ/master/plugins/Potagon_MaxLevel.js
...
@type boolean
@text 名前データベース
@desc 名前データベース(Potagon_NameDatabase.js)に対応するかの設定
@on 対応する
@off 対応しない
@default false
...
(() => {
    ...
    const MaxLevelMenu   = Potagon.convertBool(params.MaxLevelMenu);
    ...
    PluginManager.registerCommand(plugin_name, "change_level", args => {
        ...
        const show    = Potagon.convertBool(args.show);
...
Game_Actor.prototype.paramBase = function(paramId) {
    const data     = Potagon.metaData(this.actor().meta[TextManager.param(paramId)], ',');
