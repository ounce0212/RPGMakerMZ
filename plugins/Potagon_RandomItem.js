...
@target MZ
@author ポテトドラゴン
@url https://raw.githubusercontent.com/potagon/RPGMakerMZ/master/plugins/Potagon_RandomItem.js
...
(() => {
    ...
    function gacha(args) {
        ...
        const lottery     = Potagon.convertBool(args.lottery);
        ...
        if (use_lists) {
            for (let i = 0; i < use_lists.length; i++) {
                ...
                let item      = Potagon.itemSearch(use_name);
        ...
        if (lottery) {
            ...
            // ガチャをまわす
            const index = Potagon.gacha(sum, rates);
